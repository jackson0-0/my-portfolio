export default function Home() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0a1420] text-[#eef2f6] px-4 sm:px-6">
      <div className="max-w-xl w-full">
        <p className="text-sm text-[#3d6ea5] mb-2">Full Stack Developer</p>
        <h1 className="text-5xl font-bold mb-4">Hi, I'm Jackson Lam</h1>
        <p className="text-[#8a97a5] mb-8 leading-relaxed">
          A computer science student who loves building fullstack applications.
          I like building things that develop my interests and benefit others.
          I love coding and learning technologies to grow as a developer.
        </p>

        <div className="flex gap-3 mb-6">
          <a href="/contact" className="bg-[#3d6ea5] text-white px-5 py-2 rounded font-semibold hover:bg-[#4a7db4]">
            Get in Touch
          </a>
          <a href="/projects" className="border border-[#1c2d3f] text-[#eef2f6] px-5 py-2 rounded hover:border-[#3d6ea5]">
            View Projects
          </a>
        </div>

        <div className="flex gap-4">
          <a href="https://github.com/jackson0-0" target="_blank" rel="noopener noreferrer" className="text-[#8a97a5] text-sm hover:text-white">
            GitHub
          </a>
          <a href="https://linkedin.com/in/jacksonlam227" target="_blank" rel="noopener noreferrer" className="text-[#8a97a5] text-sm hover:text-white">
            LinkedIn
          </a>
        </div>
      </div>
    </div>
  );
}